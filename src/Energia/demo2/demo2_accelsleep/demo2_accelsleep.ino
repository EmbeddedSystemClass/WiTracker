...
unsigned long prevAccelReadTime;
...
void setup() {
  ...
  readDelay = 0;
  sleepMode = 0;
  startTime = prevAccelReadTime = currTime = millis();
}

void loop() {
  if ((currTime = millis()) - startTime >= 5000) {
    sleepMode = 1;
    readDelay = 5000;
  }
  
  if (sleepMode) {
    if ((currTime = millis()) - prevAccelReadTime >= readDelay) {
      acceleration();
      prevAccelReadTime = millis();
    }
  } else {
    acceleration();
  }
...
void acceleration(void)
{
  ...
  LIS.readXYZ(x, y, z);
  LIS.mgScale(x, y, z);
  if (sleepMode && (x > 100 || y > 100 || x < -100 || y < -100)) {
